...
parser.add_argument('--seq_len', type=int, default=2)#3 #example1,2
parser.add_argument('--shuffle_data', type=bool, default=False)

parser.add_argument('--cH', type=int, default=0, help='cropped image size height')
...
class Inference(Generic_train_test):
    ...
    def test(self):
        ...
        for i, data in enumerate(self.dataloader):
            ...
            if self.opts.load_1st_GS==1:
                with torch.no_grad():
                    pred_im = self.model.GS_syn(1.0-self.opts.gamma, self.opts.gamma)#corresponding to the first scanline of the second RS frame
            else:
                with torch.no_grad():
                    pred_im = self.model.GS_syn(1.0-self.opts.gamma/2.0, self.opts.gamma)#corresponding to the middle scanline of the second RS frame
            torch.cuda.synchronize()
            time_end=time.time()
            ...
            if self.opts.compute_metrics:
                ...
                ##lpips_pred=self.MSE_LossFn(self.vgg16_conv_4_3(predict_GS), self.vgg16_conv_4_3(GT_GS))
