...
parser.add_argument('--img_H', type=int, default=448, help='cropped image size height')
parser.add_argument('--crop_sz_H', type=int, default=320, help='cropped image size height')#320
...
parser.add_argument('--lr_step', type=int, default=250, help='lr decay rate')#60, 10 => 50 each lr_step epoch decay 0.5 times
...
parser.add_argument('--save_freq', type=int, default=5)#5
...
parser.add_argument('--model_label_pretrained_GS', type=str, default='pre', help='label used to pre-trained GS-based VFI model')
...
parser.add_argument('--gamma', type=float, default=1.0, help='readout time ratio')
...
class Train(Generic_train_test):
    def augment_data(self, _input):
        ...
        # Data augmentation strategy 1: horizontally flip
        ...
        for i in range(1,B):
            if torch.rand(1, 1) > 0.5:
                ...
                im_rs_1 = im_rs[i,6:9,:,:].squeeze(0)
                im_rs_4 = trans(im_rs_1).unsqueeze(0)
                im_rs_5 = torch.cat([im_rs_2,im_rs_3,im_rs_4],1)
                im_rs_ = torch.cat([im_rs_,im_rs_5],0)
            else:
        ...
        for i in range(1,B):
            if torch.rand(1, 1) > 0.5:
                ...
                im_rs_1 = im_rs[i,6:9,:,:].squeeze(0)
                im_rs_4 = trans(im_rs_1).unsqueeze(0)
                im_rs_5 = torch.cat([im_rs_4,im_rs_3,im_rs_2],1)
                im_rs_ = torch.cat([im_rs_,im_rs_5],0)
            else:
                im_rs_ = torch.cat([im_rs_,im_rs[i,:,:,:].unsqueeze(0)],0)
        im_rs=im_rs_
    ...
    def validation(self, epoch):
        ...
        for i, data in enumerate(self.dataloader_val):
            ...
            with torch.no_grad():
                pred_im, _, _, _ = self.model.GS_syn()
